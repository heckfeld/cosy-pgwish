 ...
 *
 * IDENTIFICATION
 *	  $Header: /usr/local/cvsroot/pgsql/src/interfaces/libpgtcl/pgtclCmds.c,v 1.21 1998/02/26 04:44:48 momjian Exp $
 *
 *-------------------------------------------------------------------------
 */
...
Pg_result(ClientData cData, Tcl_Interp * interp, int argc, char *argv[])
{
	...
	else if (strcmp(opt, "-cmdstat") == 0)
	{
		Tcl_AppendResult(interp, PQcmdStatus(result), 0);
		return TCL_OK;
	}
