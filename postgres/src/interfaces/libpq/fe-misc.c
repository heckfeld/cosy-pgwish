 ...
 *
 * IDENTIFICATION
 *	  $Header: /usr/local/cvsroot/pgsql/src/interfaces/libpq/fe-misc.c,v 1.10 1998/02/26 04:45:09 momjian Exp $
 *
 *-------------------------------------------------------------------------
 */
...
pqGetc(FILE *fin, FILE *debug)
{
	...
	int			c;
        ...
        while ( ((c = getc(fin)) == EOF) && (errno == EINTR))
                fprintf( stderr, "pqGetc() EINTR \n");
/*******
again:
        c = getc(fin);
