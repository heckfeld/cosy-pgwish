...
typedef struct {
    ...
    Cursor cursor;		/* Current cursor for window. */
    ...
    int titleheight;		/* Hieght of title for geometry computations */
...
static Tk_ConfigSpec configSpecs[] = {
  {TK_CONFIG_STRING, "-command", "command", "Command",
     "", Tk_Offset(Pie, command), 0},
    {TK_CONFIG_STRING, "-data", "data", "Data",
        "", Tk_Offset(Pie, userdata), 0},
    {TK_CONFIG_STRING, "-title", "title", "Title",
        "", Tk_Offset(Pie, title), 0},
  ...
  {TK_CONFIG_INT, "-explodewidth", "explodewidth", "Explodewidth",
    ...
    {TK_CONFIG_COLOR, "-textcolor", "textcolor", "Foreground",
        ...
        TK_CONFIG_MONO_ONLY},
    {TK_CONFIG_COLOR, "-piecolor", "piecolor", "Foreground",
        DEF_TEXT_COLOR, Tk_Offset(Pie, pieColor),
        TK_CONFIG_COLOR_ONLY},
    {TK_CONFIG_COLOR, "-piecolor", "piecolor", "Foreground",
        DEF_TEXT_MONO, Tk_Offset(Pie, pieColor),
        TK_CONFIG_MONO_ONLY},
     ...
     int index;
{
  ...
  if (piePtr->tkwin)
    Tk_FreeGC(Tk_Display(piePtr->tkwin),piePtr->elements[piePtr->element_count-1].gc);
}
...
#ifdef CR
  if (piePtr->cb_loc[0] < 0) {
    n = 0;
    w = 0;
    datastr = piePtr->userdata;
	...
	if (n > 0) {
	  ...
	  n = 0;
	  if (w <= 0) {
	    strcpy(machinestr, metricstr);
	  }
	  else {
	    sprintf(mmstr, "%s %s", machinestr, metricstr);    
	    value = cbval(mmstr, &(piePtr->cb_loc[w-1]));
	    pieInsert(piePtr, metricstr, value);
	  }
	  w++;
	}
	if (c == '\0')
	...
	metricstr[n++] = datastr[i];
      }
    }
  }
  else {
    for (w = 0; w <= piePtr->element_count; w++) {
      value = cbval(0, &(piePtr->cb_loc[w]));
      alterPieIndex(piePtr, w, value);
    }
  }
#endif
     ...
     char **argv;
{
  ...
  if (!piePtr) {
    fprintf( stderr, "Failed to allocate %ld bytes for the Pie structure\n",
	    sizeof(Pie) );
    return TCL_ERROR;
    }
