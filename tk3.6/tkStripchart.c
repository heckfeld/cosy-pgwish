...
#define DEF_STRIPCHART_TITLE               NULL
...
#define DEF_USERDATA                       NULL
#define DEF_GUARENTEE_DRAW                 "false"
...
typedef struct {
    ...
    Tk_TimerToken cb_timer;   /* Token for callback timer. */
...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_STRING, "-command", "command", "Command",
        NULL, Tk_Offset(Stripchart, command), 0},
    ...
    {TK_CONFIG_ACTIVE_CURSOR, "-cursor", "cursor", "Cursor",
     NULL, Tk_Offset(Stripchart, cursor), TK_CONFIG_NULL_OK},
    ...
    {TK_CONFIG_BOOLEAN, "-guarenteedrawing", "guarenteedrawing",
        "Guarenteedrawing", DEF_GUARENTEE_DRAW,
    ...
    char **argv;		/* Argument strings. */
{
    ...
    StripchartPtr->cb_loc = -1;
    StripchartPtr->cb_key = 0;
    StripchartPtr->cb_timer = NULL;
    ...
    char **argv;		/* Argument strings. */
{
  ...
  if ((c == 'c') && (strncmp(argv[1], "configure", length) == 0)) {
    ...
    StripchartPtr->continue_callback = FALSE;
    if (StripchartPtr->cb_timer) {
      Tk_DeleteTimerHandler(StripchartPtr->cb_timer);
      StripchartPtr->cb_timer = NULL;
    }
     ...
     Stripchart *StripchartPtr;
{
  ...
  if ( StripchartPtr->command != NULL && StripchartPtr->command[0] != '\0' ) {
    ...
    if ( result != TCL_OK ) {
      Tk_BackgroundError(StripchartPtr->interp);
      no_errors = FALSE;
    }
  }

#ifdef CR
  SetStripchartValue(StripchartPtr, cbval(StripchartPtr->userdata, &(StripchartPtr->cb_loc)));
#endif

  if ( StripchartPtr->continue_callback && no_errors )
    StripchartPtr->cb_timer =
      Tk_CreateTimerHandler(StripchartPtr->interval,
			    (Tk_TimerProc *)Callback,
...
/*mike    if (StripchartPtr->value != NULL)
      free(StripchartPtr->value);*/

    if (StripchartPtr->cursor != None)
      Tk_FreeCursor(Tk_Display(StripchartPtr->tkwin), StripchartPtr->cursor);
    ...
    if ((StripchartPtr->textGC != None) && StripchartPtr->tkwin)
        Tk_FreeGC(Tk_Display(StripchartPtr->tkwin),StripchartPtr->textGC);
    ...
    if ((StripchartPtr->tickGC != None) && StripchartPtr->tkwin)
        Tk_FreeGC(Tk_Display(StripchartPtr->tkwin),StripchartPtr->tickGC);
    ...
    register XEvent *eventPtr;	/* Information about event. */
{
  ...
  if ((eventPtr->type == Expose) && (eventPtr->xexpose.count == 0)) {
    ...
    if (StripchartPtr->cb_timer)
      Tk_DeleteTimerHandler(StripchartPtr->cb_timer);
    StripchartPtr->cb_timer = NULL;
    StripchartPtr->continue_callback = 0;
   ...
   int displaybits;
{
  ...
  if (StripchartPtr->guarentee_draw)
