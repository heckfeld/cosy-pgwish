    ...
    int curve_format;
{
	...
	switch (gRT_out_format) {
		...
		*/
	    ...
	    case OUTPUT_SCED:
		if (ABSOLUTE(apex_pt[W] - base_pt[W]) < EPSILON)
		    fprintf(gOutfile, "Object \"cylinder%d\" \"cylinder\"\n",
			sced_index++);
		else
		    fprintf(gOutfile, "Object \"cone%d\" \"cone\"\n",
			sced_index++);
    ...
    int curve_format;
{
	...
	switch (gRT_out_format) {
	    ...
	    case OUTPUT_SCED:
		fprintf(gOutfile, "Object \"disc%d\" \"disc\"\n", sced_index++);
		if (lib_tx_active())
		   lib_output_tx_sequence();
		fprintf(gOutfile, "\n");
		break;
    ...
    int curve_format;
{
	...
	switch (gRT_out_format) {
	    ...
	    case OUTPUT_SCED:
		fprintf(gOutfile, "Object \"sphere%d\" \"sphere\"\n",
				sced_index);
		fprintf(gOutfile, " Scale %g %g %g\n",
	    ...
	    case OUTPUT_VRML:
		fprintf(gOutfile, "TransformSeparator {\n");
		lib_tx_push();
		lib_tx_translate(center_pt);
		lib_output_tx_sequence();
		lib_tx_pop();
		/*fprintf(gOutfile, "\tTranslation {\n");
		fprintf(gOutfile, "\t\translation %g %g %g\n", center_pt[X],center_pt[Y],center_pt[Z]);
		fprintf(gOutfile, "\t}\n");*/
    ...
    COORD3 p1, p2;
{
	...
	switch (gRT_out_format) {
	    ...
	    case OUTPUT_SCED:
		fprintf(gOutfile, " Object \"cube%d\" \"cube\"\n");
		fprintf(gOutfile, " Scale %g %g %g",
