    ...
    int    resx, resy;
{
	...
	case OUTPUT_SCED:
	    fprintf(gOutfile, "Viewport\n");
	    fprintf(gOutfile, "    LookFrom %g %g %g\n", from[X], from[Y], from[Z]);
	    fprintf(gOutfile, "    LookAt %g %g %g\n", at[X], at[Y], at[Z]);
	    fprintf(gOutfile, "    LookUp %g %g %g\n", up[X], up[Y], up[Z]);
	...
	case OUTPUT_VRML:
	    ...
	    fprintf(gOutfile, "\tposition\t%g %g %g\n",
				from[X],from[Y],from[Z]);
	{
		Convert_Camera_Model(from, at, up, &axis, &rot_angle);
		fprintf(gOutfile, "\torientation\t%g %g %g %g\n",
			axis[X],axis[Y], axis[Z],rot_angle);
	}
    ...
    COORD4 center_pt;
{
	...
	case OUTPUT_SCED:
	    fprintf(gOutfile, "Object \"light%d\" \"light\"\n",
				sced_index++);
    ...
    COORD3 color;
{
	...
	case OUTPUT_VRML:
	    fprintf(gOutfile, "DEF BackgroundColor Info { string \"%g %g %g\"}\n",
		    color[X], color[Y], color[Z]);
	    break;
    ...
    double ka, kd, ks, shine, ang, kt, i_of_r;
{
	...
	case OUTPUT_SCED:
	    ...
	    fprintf(gOutfile, "    Specular %g %g\n", shine, phong_pow);
	    fprintf(gOutfile, "    Reflect %g\n", ks);
	    fprintf(gOutfile, "    Refract %g\n", i_of_r);
	...
	case OUTPUT_VRML:
	    ...
	    if (shine != 0) {
		fprintf(gOutfile, "\tspecularColor 1.0 1.0 1.0\n");
		...
		if (shine == 0.0) {
		    /* If there is no Phong highlighting, but there is
		       ...
		       specular reflections */
		    fprintf(gOutfile, "\tspecular 1.0 1.0 1.0\n");
		    fprintf(gOutfile, "\tshininess 0.0\n");
		}
		/*fprintf(gOutfile, "reflect %g\n", ks);*/
	    }
