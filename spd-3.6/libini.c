...
char    *filename ;     /* unused except for Mac version */
{
    ...
    else if (raytracer_format == OUTPUT_SCED) {
       fprintf(gOutfile, "Version 0.8\n");
       lib_set_raytracer(raytracer_format);
       }
    else if (raytracer_format == OUTPUT_VRML) {
...
lib_close PARAMS((void))
{
    ...
    else if (gRT_out_format == OUTPUT_VRML) {
	fprintf(gOutfile, "}\n");
    }
...
void show_gen_usage PARAMS((void))
{
    ...
    fprintf(stderr, "   16  ScED External file format\n");
    fprintf(stderr, "   17  VRML file format\n");
